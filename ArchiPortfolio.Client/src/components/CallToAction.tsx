...
const CallToAction: React.FC<CallToActionProps> = ({ title, description, buttonText, email }) => {
  return (
    <section className="py-32 px-6 md:px-16 bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white transition-colors duration-500 border-t border-zinc-200 dark:border-white/5">
      <div className="max-w-4xl mx-auto flex flex-col items-center text-center">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-4xl md:text-5xl font-bold mb-6">{title || "Ready to build?"}</h2>
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <a href={`mailto:${email || 'hello@vivere.design'}`} className="text-zinc-500 text-sm hover:text-accent-600 dark:hover:text-white transition-colors">
              or email us at <span className="underline decoration-zinc-300 dark:decoration-zinc-700 underline-offset-4">{email || 'hello@vivere.design'}</span>
            </a>
          </div>
        </motion.div>
